#!/bin/bash
# ============================================================================
# Reset Cassandra Cluster & Load Initial Fantasy Game Schema
# ============================================================================
# Stops the local docker compose Cassandra cluster, removes data volumes so
...
#   INIT_SCRIPT=INIT_fantasy_game_setup.cql ./reset_and_init_cluster.sh
#
# Requirements:
#   • docker & docker compose installed
#   • Cluster defined in ./docker compose.yaml with container names:
#       fantasy-cassandra-node1, fantasy-cassandra-node2, fantasy-cassandra-node3
# ----------------------------------------------------------------------------
set -euo pipefail

COMPOSE_FILE="docker-compose.yaml"
INIT_SCRIPT=${INIT_SCRIPT:-"INIT_fantasy_game_setup.cql"}
CONTAINERS=(fantasy-cassandra-node1 fantasy-cassandra-node2 fantasy-cassandra-node3)
...
# -----------------------------------------------------------------------------
info "Starting Cassandra nodes …"
docker compose -f "$COMPOSE_FILE" up -d

# -----------------------------------------------------------------------------
# 2b. Wait until *all* Cassandra nodes are reachable on CQL port
# -----------------------------------------------------------------------------
wait_for_cql() {
  local node=$1 max=60
...
info "Waiting for Cassandra to finish bootstrapping …"
